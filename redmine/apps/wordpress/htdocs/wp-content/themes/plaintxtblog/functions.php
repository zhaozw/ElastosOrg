...
?>
		<?php echo $before_widget; ?>
			<?php echo $before_title . $title . $after_title; ?>
			<ul>
				<li><a href="<?php bloginfo('rss2_url') ?>" title="<?php echo _wp_specialchars( get_bloginfo('name'), 1 ) ?> <?php _e( 'Posts RSS feed', 'plaintxtblog' ); ?>" rel="alternate" type="application/rss+xml"><?php _e( 'All posts', 'plaintxtblog' ) ?></a></li>
				<li><a href="<?php bloginfo('comments_rss2_url') ?>" title="<?php echo _wp_specialchars(bloginfo('name'), 1) ?> <?php _e( 'Comments RSS feed', 'plaintxtblog' ); ?>" rel="alternate" type="application/rss+xml"><?php _e( 'All comments', 'plaintxtblog' ) ?></a></li>
			</ul>
		<?php echo $after_widget; ?>
<?php
}
	...
	<form action="<?php echo _wp_specialchars( $_SERVER['REQUEST_URI'] ) ?>" method="post">
		...
		<table class="form-table" summary="PlaintxtBlog typography options">
			<tr valign="top">
				...
				<td>
					<input id="ptb_basefontsize" name="ptb_basefontsize" type="text" class="text" value="<?php if ( get_option('plaintxtblog_basefontsize') == "" ) { echo "70%"; } else { echo esc_attr( get_option('plaintxtblog_basefontsize') ); } ?>" tabindex="1" size="10" />
		...
		<table class="form-table" summary="PlaintxtBlog content options">
			<tr valign="top">
				...
				<td>
					<input id="ptb_avatarsize" name="ptb_avatarsize" type="text" class="text" value="<?php if ( get_option('plaintxtblog_avatarsize') == "" ) { echo "24"; } else { echo esc_attr( get_option('plaintxtblog_avatarsize') ); } ?>" size="6" />
...
function plaintxtblog_wp_head() {
	if ( get_option('plaintxtblog_basefontsize') == "" ) {
		$basefontsize = '70%';
			...
			$basefontsize = esc_attr( stripslashes( get_option('plaintxtblog_basefontsize') ) );
	};
	if ( get_option('plaintxtblog_basefontfamily') == "" ) {
		$basefontfamily = 'verdana, geneva, sans-serif';
		} else {
			$basefontfamily = _wp_specialchars( stripslashes( get_option('plaintxtblog_basefontfamily') ) );
	};
	if ( get_option('plaintxtblog_headingfontfamily') == "" ) {
		$headingfontfamily = 'verdana, geneva, sans-serif';
		} else {
			$headingfontfamily = _wp_specialchars( stripslashes( get_option('plaintxtblog_headingfontfamily') ) );
	};
	if ( get_option('plaintxtblog_posttextalignment') == "" ) {
		$posttextalignment = 'left';
		} else {
			$posttextalignment = esc_attr( stripslashes( get_option('plaintxtblog_posttextalignment') ) );
	};
...
body div.sidebar{border-left:5px solid #ddd;padding-left:15px;}';
	...
	if ( get_option('plaintxtblog_sidebartextalignment') == "" ) {
		$sidebartextalignment = 'right';
			...
			$sidebartextalignment = esc_attr( stripslashes( get_option('plaintxtblog_sidebartextalignment') ) );
	};
