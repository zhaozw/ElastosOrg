...
function invites_beautify($str)
{
	...
	$user = substr($str, $wp_invites_options['INVITE_LENGTH']);
	$str = substr($str, 0, $wp_invites_options['INVITE_LENGTH']);

	$str = implode($wp_invites_options['SEPARATOR'], str_split($str, $wp_invites_options['INVITE_SPLIT']));
...
function invites_add_admin_menu()
{
	...
	if(constant('IS_BUDDYPRESS')) {
		add_submenu_page(/* 'bp-general-settings'*/'plugins.php', 'WP-invites', 'WP-invites', 8, "manage_invites", "invites_admin" );
...
function wp_output_invites($user)
{
	...
	$sql = 'SELECT meta FROM ' . INVITES_PREFIX . 'signups WHERE `user_login`="' . $user->user_login . '"';
	$meta = $wpdb->get_results($sql, ARRAY_A);
	$code = unserialize($meta[0]['meta']);
	$code = invites_beautify($code['invite_code']);

	if(!$code)
		$code=__('No code assigned', 'wp-invites');

	if ( !function_exists('is_site_admin') || (is_site_admin() || (get_current_user_id() == $user->ID)) ) { ?>
<table class="form-table">
<tr>
	<th>
<label for="invite code"><?php _e('Invite code', 'wp-invites') ?></label></th>
	<td><input type="text"  disabled="disabled" class="regular-text" value="<?php echo $code; ?>"></td></tr></table>
	<?php
	}
}
...
if(constant('IS_BUDDYPRESS'))
{  #add registration field in buddy
  ...
  add_filter('bp_signup_usermeta','wpmu_invites_add_signup_meta',1,1);
	...
	/* Xilong Pei
	 *
	 * this doesn't work, this action should be used:
	 * do_action( 'bp_core_activated_user', $user_id, $key, $user )
	 *
	 * BUT, all of them is no use now, I needn't the information in user_meta'
	 *
