...
function bp_message_attachement_add_attachement($msg) {
	...
	if ($_FILES[$f_u]['name']!="" && $_FILES[$f_u]['error'] != UPLOAD_ERR_NO_FILE){
		...
		$upload_folder = $upload_directory['basedir'] . "/message-attachements/$y/$m";
		...
		if ( $_FILES[$f_u]["size"] < $max_allwd_size && in_array($extension, $allowedExts)){
			...
			if ($_FILES[$f_u]["error"] > 0) {
				...
				if (file_exists($upload_folder . "/" . $org_name)) {
					...
					move_uploaded_file($_FILES[$f_u]["tmp_name"], $upload_folder . "/" . $org_name);

					//file uploaded successfuly now save the custom post
					$pst_content = "<a href='" . $upload_directory['baseurl'] . "/message-attachements/" . $org_name . "' title='view/Download' class='msgat_file'>" . $_FILES[$f_u]["name"] . "</a>";
...
function bp_message_attachement_delete_attachement($thread_ids) {
	global $wpdb, $bp;
	...
	for ( $i = 0, $count = count($thread_ids);  $i < $count;  ++$i ) {
		...
		$threads = $wpdb->query( $wpdb->prepare("SELECT * FROM {$bp->messages->table_name_messages} WHERE thread_id = %d", $thread_ids[$i]) );
		if ( empty($threads) )
			continue;

		foreach ( (array)$threads as $key => $thread_template ) {
			$params = array(
				'author' 		=> $thread_template->sender_id,
				'post_type'		=> 'messageattachements',
				'meta_key' 		=> 'bp_msgat_message_id',
				'meta_value'	=> $thread_template->thread_id ."=".$thread_template->date_sent
			);
			$the_query = new WP_Query( $params );
			if ($the_query->have_posts() ) {
				while ( $the_query->have_posts() ) {
					...
					//remove the atachment
					$the_file = get_the_excerpt();
					$org_name = strstr($the_file, '/message-attachements/');
					unlink($upload_directory . $the_file);
...
add_action('messages_before_delete_thread', 'bp_message_attachement_delete_attachement');
?>