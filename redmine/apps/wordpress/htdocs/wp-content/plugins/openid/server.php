...
function openid_provider_xrds_simple($xrds) {
	...
	//elastos.org
	// if get_option('openid_blog_owner') --> false
	// use the first user as its owner
	if (!$user) {
		//get the userID, it is login 
		$users = get_users_of_blog();

		if (!empty($users)) {

			//$str is a string like: http://xilongpei.elastos.org/index.php/openid/server
			$str = openid_server_url();

			foreach ($users as $usr) {
				if (stripos($str, $usr->user_login . '.') == 7) {
					$user = get_userdatabylogin($usr->user_login);
					break;
				}
			}
		}
