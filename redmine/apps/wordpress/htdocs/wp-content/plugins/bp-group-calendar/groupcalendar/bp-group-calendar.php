...
function bp_group_calendar_js() {
	...
	if (!is_admin() && strpos($_SERVER['REQUEST_URI'], '/calendar/') !== false) {
		...
		wp_enqueue_style( 'jquery-datepicker-css', plugins_url('/bp-group-calendar/groupcalendar/datepicker/css/ui-lightness/jquery-ui-1.7.2.custom.css'), false);
		wp_enqueue_script( 'jquery-datepicker', plugins_url('/bp-group-calendar/groupcalendar/datepicker/js/datepicker.min.js'), array('jquery', 'jquery-ui-core'));

		//only load languages for datepicker if not english (or it will show Chinese!)
		if ($bgc_locale['code'] != 'en')
			wp_enqueue_script( 'jquery-datepicker-i18n', plugins_url('/bp-group-calendar/groupcalendar/datepicker/js/jquery-ui-i18n.min.js'), array('jquery', 'jquery-ui-core', 'jquery-datepicker'));
	}
}
