...
$label_string .= ($si_captcha_opt['si_captcha_label_captcha'] != '') ? $si_captcha_opt['si_captcha_label_captcha'] : __('Prefix 4 CAPTCHA Code', 'si-captcha');
$label_string .= '</label>';
...
<p>
 <label>';
  echo ($si_captcha_opt['si_captcha_label_captcha'] != '') ? $si_captcha_opt['si_captcha_label_captcha'] : __('Prefix 4 CAPTCHA Code', 'si-captcha');
  echo '<br />
   ...
   //captcha without sessions
      if (empty($_POST['captcha_code']) || $_POST['captcha_code'] == '') {
         ...
         if ( is_readable( $si_captcha_dir_ns . $prefix . '.php' ) ) {
			...
			$str = trim(strip_tags($_POST['captcha_code']));
			if ( (strpos($captcha_word, $str) !== false) && (strlen($str)==4) ) {
             ...
             if($unlink == 'unlink')
              ...
              return 'valid';
               ...
               return ($si_captcha_opt['si_captcha_error_incorrect'] != '') ? $si_captcha_opt['si_captcha_error_incorrect'] : __('Wrong CAPTCHA! we just need prefix 4 characters of CAPTCHA code', 'si-captcha');
            }
	     } else {
