...
//        $results = $wpdb->get_results($query);
    ...
    public function add_menu() {
        $page = add_options_page('WP Microblogs', 'WP Microblogs', 'manage_options', 'wp-microblogs', array($this, 'option_page'));
    ...
    public function option_page() {
        ...
        <div class="wrap">
            ...
            <?php
//            wm_debug_print(get_option('widget_wm_show'));
            $nextrun = wp_next_scheduled('wm_update_timeline');
            if (!$nextrun ) {
                if (empty($nextrun) || $nextrun < (time()-(3600*24))) {
            ?>
            <div class="error"><p><strong>看起来 WP-Cron 没有正常工作。</strong><br />WP Microblogs 需要 WP-Cron 进行自动更新。</p></div>
            <?php
                }
            }
            ...
            if (isset($_POST['update'])) {
                ?>
                <div class="updated"><p><strong id="cache-update">缓存更新中，请不要关闭这个页面……</strong></p></div>
                <?php
                wp_ob_end_flush_all();
                flush();
                MicroblogOption::clearLasttime();
                wm_delete_cache(-1);
                do_action('wm_update_timeline');
                wm_cleanup();
                ?>
                <script type="text/javascript">
                    document.getElementById('cache-update').innerHTML="缓存已更新。";
                    <?php if (!WM_DEBUG) { ?>setTimeout("window.location.href = window.location.href;", 2000);<?php } ?>
                </script>
                <?php
                return;
            }
                    ...
                    <table class="form-table">
                        <tr valign="top">
                            ...
                            <td>
                                ...
                                ?>
                                <label for="count">
                                    每个帐号最多保存 <input type="text" name="count" id="count" class="small-text" value="<?php echo $options['count']; ?>"> 条推文 <span class="description">0 表示不限制</span>
                                </label>
                            </td>
                        </tr>
                        <tr valign="top">
                            ...
                            <td>
                                <label for="unique">
                                    <input type="checkbox" name="unique" id="unique" value="1"<?php echo $options['unique'] == 1 ? ' checked="checked"' : ''; ?>> 过滤重复推文
                                </label><br />
                                <label for="similar">
                        ...
                        <tr valign="top">
                            ...
                            <td>
                                <label for="autolink">
                                    <input type="checkbox" name="autolink" id="autolink" value="1"<?php echo $options['autolink'] == 1 ? ' checked="checked"' : ''; ?>> 为推文中提到的 URL 添加链接
                                </label>
                            </td>
                        </tr>
                        <tr valign="top">
                            ...
                            <td>
                                <label for="uninstall">
                                    ...
                                    如果是因为无法添加微博帐号、获得推文等原因准备卸载插件，希望您能反馈到<a href="http://beamnote.com/2011/wp-microblogs.html#respond">插件主页</a>，谢谢。
                                </label>
                            </td>
                        </tr>
                    </table>
    ...
    public static function getAccounts($nosuspend = false) {
    ...
    public static function clearLasttime() {
        $accounts = self::getOrigAccounts();
        foreach ($accounts['accounts'] as $key => $account)
            $accounts['accounts'][$key]['lasttime'] = NULL;
        update_option('wm_accounts', $accounts);
        return true;
    }
