<?php /* Smarty version 2.6.26, created on 2013-06-26 16:41:28
         compiled from execute/bugAdd.tpl */ ?>
<?php require_once(SMARTY_CORE_DIR . 'core.load_plugins.php');
smarty_core_load_plugins(array('plugins' => array(array('modifier', 'basename', 'execute/bugAdd.tpl', 7, false),array('modifier', 'replace', 'execute/bugAdd.tpl', 7, false),array('modifier', 'escape', 'execute/bugAdd.tpl', 23, false),array('function', 'config_load', 'execute/bugAdd.tpl', 8, false),array('function', 'lang_get', 'execute/bugAdd.tpl', 12, false),)), $this); ?>
<?php $_smarty_tpl_vars = $this->_tpl_vars;
$this->_smarty_include(array('smarty_include_tpl_file' => "inc_head.tpl", 'smarty_include_vars' => array()));
$this->_tpl_vars = $_smarty_tpl_vars;
unset($_smarty_tpl_vars);
 ?>

<?php $this->assign('cfg_section', ((is_array($_tmp=((is_array($_tmp='execute/bugAdd.tpl')) ? $this->_run_mod_handler('basename', true, $_tmp) : basename($_tmp)))) ? $this->_run_mod_handler('replace', true, $_tmp, ".tpl", "") : smarty_modifier_replace($_tmp, ".tpl", ""))); ?>
<?php echo smarty_function_config_load(array('file' => "input_dimensions.conf",'section' => $this->_tpl_vars['cfg_section']), $this);?>


<body onunload="dialog_onUnload(bug_dialog)" onload="dialog_onLoad(bug_dialog)">
<h1 class="title">
	<?php echo lang_get_smarty(array('s' => 'title_bug_add'), $this);?>
 
	<?php $_smarty_tpl_vars = $this->_tpl_vars;
$this->_smarty_include(array('smarty_include_tpl_file' => "inc_help.tpl", 'smarty_include_vars' => array('helptopic' => 'hlp_btsIntegration','show_help_icon' => true)));
$this->_tpl_vars = $_smarty_tpl_vars;
unset($_smarty_tpl_vars);
 ?>
</h1>

<?php $_smarty_tpl_vars = $this->_tpl_vars;
$this->_smarty_include(array('smarty_include_tpl_file' => "inc_update.tpl", 'smarty_include_vars' => array('user_feedback' => $this->_tpl_vars['gui']->msg)));
$this->_tpl_vars = $_smarty_tpl_vars;
unset($_smarty_tpl_vars);
 ?>
<div class="workBack">
	<form action="lib/execute/bugAdd.php" method="post">
		<input type="hidden" name="tproject_id" id="tproject_id" value="<?php echo $this->_tpl_vars['gui']->tproject_id; ?>
">
		<?php if ($this->_tpl_vars['gui']->user_action == 'link'): ?>
	  	<p>
			<a style="font-weight:normal" target="_blank" href="<?php echo $this->_tpl_vars['gui']->createIssueURL; ?>
">
			<?php echo lang_get_smarty(array('s' => 'link_bts_create_bug'), $this);?>
(<?php echo ((is_array($_tmp=$this->_tpl_vars['gui']->issueTrackerVerboseID)) ? $this->_run_mod_handler('escape', true, $_tmp) : smarty_modifier_escape($_tmp)); ?>
)</a>
		</p>	
	  	<p class="label"><?php echo ((is_array($_tmp=$this->_tpl_vars['gui']->issueTrackerVerboseType)) ? $this->_run_mod_handler('escape', true, $_tmp) : smarty_modifier_escape($_tmp)); ?>
 <?php echo lang_get_smarty(array('s' => 'bug_id'), $this);?>

  	 		<input type="text" id="bug_id" name="bug_id" size="<?php echo $this->_config[0]['vars']['BUGID_SIZE']; ?>
" maxlength="<?php echo $this->_tpl_vars['gui']->bugIDMaxLength; ?>
"/>
		</p>	
		<?php endif; ?>
		<div class="groupBtn">
     <?php if ($this->_tpl_vars['gui']->user_action == 'link'): ?>
			<input type="submit" value="<?php echo lang_get_smarty(array('s' => 'btn_add_bug'), $this);?>
" onclick="return dialog_onSubmit(bug_dialog)" />
     <?php endif; ?> 
			<input type="button" value="<?php echo lang_get_smarty(array('s' => 'btn_close'), $this);?>
" onclick="window.close()" />
		</div>
 	</form>
</div>

</body>
</html>