...
SVN_PIDFILE=/opt/redmine/subversion/tmp/svnserve.pid

SVN_START="/opt/redmine/subversion/bin/svnserve -d --listen-port=3690 --pid-file=$SVN_PIDFILE"
...
start_subversion() {
    ...
    if [ $ERROR -eq 0 ]; then
	echo "$0 $ARG: subversion started at port 3690"
	sleep 2
    else
